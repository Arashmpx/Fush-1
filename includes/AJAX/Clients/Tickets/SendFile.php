...
$ticket_id = $Sec->input($_SESSION['ticketid']);
    $upload = new Upload();
    $response_data = $upload->handleFileUpload($_FILES['file'],"TicketFile/","File");
	$response_data_decode = json_decode($response_data);
	$response = $response_data_decode->response;
	$file = $response_data_decode->filename;
	$filetype = $response_data_decode->filetype;
	$fileSize = $response_data_decode->fileSize;
...
if(!empty($file)){
$MessageSize = "[[".round($fileSize);
}else{
   $MessageSize = NULL; 
}
$data = array(
    'vtr_uid' => $current_user_id,      
    'vtr_message' => $file.$MessageSize,  
    'vtr_time' => $date,      
    'vtr_ticket_id' => $ticket_id,  
    'vtr_msg_type' => $filetype  ,
